...
from app.worker import _step_numpy, is_oscillating_match, filter_highlights_by_oscillation
import numpy as np
...
def test_step_birth_majority_red():
    ...
    from app.worker import _step_numpy, _TEAM_RED, _TEAM_BLUE
...
def test_stable_block_not_oscillator():
    """A 2x2 still-life block is static (period-1), not a period-2..5 oscillator → False."""
    red_seed = [0] * 64
...
def _make_highlight(blinker_in_red: bool, blinker_in_blue: bool, name: str, score: float) -> dict:
    rs = [0] * 64
    bs = [0] * 64
    if blinker_in_red:
    ...
    return {"red_seed": rs, "blue_seed": bs,
            "red_name": name, "blue_name": "Opponent",
            "metric_value": score}
...
def test_filter_non_oscillating_first():
    ...
    assert result[0]["red_name"] == "dynamic",    f"Expected 'dynamic' first, got {result[0]['red_name']}"
    assert result[1]["red_name"] == "oscillator", f"Expected 'oscillator' second, got {result[1]['red_name']}"
    print("PASS: test_filter_non_oscillating_first")


def test_filter_preserves_order_within_groups():
    """Within each group, higher metric_value must come first (input order preserved)."""
    highlights = [
    ...
    result = filter_highlights_by_oscillation(highlights, 200)
    assert result[0]["red_name"] == "dyn_high", f"Expected 'dyn_high' at [0], got {result[0]['red_name']}"
    assert result[1]["red_name"] == "dyn_low",  f"Expected 'dyn_low'  at [1], got {result[1]['red_name']}"
    assert result[2]["red_name"] == "osc_high", f"Expected 'osc_high' at [2], got {result[2]['red_name']}"
    assert result[3]["red_name"] == "osc_low",  f"Expected 'osc_low'  at [3], got {result[3]['red_name']}"
...
def test_filter_all_oscillating_returns_all():
    ...
    highlights = [_make_highlight(True, False, f"osc_{i}", float(9000-i*100)) for i in range(4)]
    result = filter_highlights_by_oscillation(highlights, 200)
