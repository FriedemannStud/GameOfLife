...
- [x] **Step 1.1: Create the duel page skeleton**
    - [x] **Action:** Create `web/duel/duel.html` as a minimal mobile-first page with a single
    ...
    - [x] **Action:** In `backend/app/main.py`, mount it next to the editor:
        `app.mount("/duel", StaticFiles(directory="web/duel"), name="duel")`. Mark with
        `# KI-Agent unterstützt`.
    - [x] **Verification:** Run `uvicorn app.main:app --reload --port 8000` from `backend/`, open
...
- [x] **Step 1.2: Self-host the Suno soundtrack**
    - [x] **Action:** Download the project's Suno track ("Open Flow") as MP3 and place it at
    ...
    - [x] **Verification:** Open `http://localhost:8000/assets/duel_theme.mp3` (add an `/assets`
...
- [x] **Step 2.1: Add duel models**
    - [x] **Action:** In `backend/app/models.py`, add `DuelParticipant`, `DuelRoom`, and the
    ...
    - [x] **Verification:** `cd backend && python -c "from app.models import DuelRoom, DuelParticipant; print('ok')"`.
        **Expected Result:** Prints `ok`, no import errors.

- [x] **Step 2.2: Duel service — create & join with TTL**
    - [x] **Action:** Create `backend/app/duel_service.py` with `create_room` and `join_room` per
    ...
    - [x] **Action:** In `main.py` startup, create a **TTL index** on `duel_rooms.expires_at`
        (`expireAfterSeconds=0`) and a unique index on `duel_rooms.room_id`.
    - [x] **Verification:** Add `backend/tests/test_duel_room.py` (self-running, unique ids,
...
- [x] **Step 2.3: Duel router — room endpoints + state poll**
    - [x] **Action:** Create `backend/app/duel_router.py` (an `APIRouter`, prefix `/api/v1/duel`)
    ...
    - [x] **Action:** Follow the ADR-0027 error contract: deterministic `404`/`409`/`410`, no
        catch-all `500` swallowing the conflict paths.
    - [x] **Verification:** With the backend running:
        ```
...
- [x] **Step 3.1: Own-configs endpoint**
    - [x] **Action:** Add `GET /api/v1/my_configs?player_id=` (in `duel_router.py`), returning the
        player's active `submissions` (`metadata.player_id == player_id`) as
        `{config_id, nickname, seed}` using `cells_to_grid` for the seed icon.
    - [x] **Verification:** Submit a config via the editor for a known `player_id`, then
...
- [x] **Step 3.2: Lock-in with hidden choice + race guard**
    - [x] **Action:** Add `POST /rooms/{room_id}/lock` accepting `{player_id, config | config_id}`.
    ...
    - [x] **Verification:** Extend `test_duel_room.py`: lock A (status stays `choosing`, opponent
...
- [x] **Step 3.3: Run the headless referee**
    - [x] **Action:** In `duel_service.py`, add `run_match(room)` mirroring `worker.py:160-185`:
    ...
    - [x] **Verification:** Ensure binaries are built (`make`). Drive a full create→join→lock→lock
...
- [x] **Step 3.4: Persist the duel record**
    - [x] **Action:** In `run_match`, after computing, insert a `duels` document per
    ...
    - [x] **Verification:** Run a full duel, then inspect Mongo:
...
- [x] **Step 4.1: Record aggregation endpoint**
    - [x] **Action:** Add `GET /api/v1/duels?player_id=` (optional `recovery_code`) calling
    ...
    - [x] **Verification:** Add `backend/tests/test_duel_record.py` seeding a few `duels` docs and
...
- [x] **Step 5.1: Pairing UI (QR + room code)**
    - [x] **Action:** In `duel.html`, wire "Start Duel" → `POST /rooms`, render a **QR code** of the
        ...
        on `GET /rooms/{id}`.
        _(Implemented as a QR deep-link `…/duel.html?join=ROOM_ID` scanned by the native camera +
        a 4-char manual code via `GET /rooms/by-code/{code}`; no in-app scanner library.)_
...
- [x] **Step 5.2: Choice UI (own configs + quick-draw)**
    - [x] **Action:** Fetch `GET /api/v1/my_configs`, render the player's configs as selectable
...
- [x] **Step 5.3: Result screen + Rematch**
    - [x] **Action:** When the poll sees `status:"result"`, show a **WIN/LOSE** screen (compare the
...
- [x] **Step 5.4: "My Duels" view (local-first)**
    - [x] **Action:** After each result, append to `localStorage['biotope_duels']`. Add a "My Duels"
...
- [x] **Step 6.1: VS splash + soundtrack + Suno credit**
    - [x] **Action:** Add a VS splash (both seeds + names) shown before the result, with the
...
- [x] **Step 6.2: Deterministic replay** _(delivered via the server frame-payload path, not WASM —
    `emcc` unavailable in the dev shell; ADR-0028 §2.2 fallback. See CHANGELOG.)_
    - [x] **Action:** ~~Load the existing WASM core (`Makefile.wasm` output) in `duel.html`~~;
        between the VS splash and the result, animate the match by re-running the deterministic
        simulation from both seeds, with a live red/blue population bar. **The displayed winner stays
        the server verdict.** Fallback used: `biotope_headless` emits 101 per-generation frames
        (8×16 "0/1/2" strings) via an opt-in argv path; `duel_service` serves them once via
        `GET /rooms/{id}/frames`; `duel.html` animates them on a canvas, clock-locked to the
        soundtrack (~120 BPM → 1/16-note step, audio-clock-driven) with a proportional population
        meter and a "Gen N / 100" counter + timeline.
    - [x] **Verification (Interactive Test):**
        1. Play the same pair of configs twice.
        2. **Expected Result:** The animation looks identical on both phones and across both runs;
           the final on-screen winner equals the server `winner`. (Verified OK by the developer.)

- [x] **Step 6.3: Guest fallback & post-match claim prompt**
    - [x] **Action:** Ensure a phone with no claimed name plays as `Gast-XXXX` (FR-4). After a guest's
        ...
        _(Bug found & fixed during verification: `editor.html` now writes
        `localStorage['biotope_nickname']` on a successful claim, so the same-browser duel page
        reuses the claimed name instead of `Gast-XXXX`.)_
    - [x] **Verification (Interactive Test):**
...
- [x] **Step 7.1: Backend test sweep**
    - [x] **Action:** Ensure `test_duel_room.py` and `test_duel_record.py` cover the happy path and
        the key edge cases (full room, hidden choice, double-lock race, wrong recovery code). Confirm
        existing tests still pass.
    - [x] **Verification:** Run each `backend/tests/test_*.py` and confirm no regressions; run
...
- [x] **Step 7.2: End-to-end integration test**
    - [x] **Action:** Add a `test_duel_integration.py` that, against a running backend with built
        ...
        `test_system_integration.py`). _(Also asserts the non-empty 8×16 / 101-frame replay payload.)_
    - [x] **Verification:** `make` first, then run the integration test.
        **Expected Result:** The full duel completes and the record is written. _(Passes in
        `gameoflife-backend-1`.)_

- [ ] **Step 7.3: Final interactive acceptance** _(pending — developer to run the full loop at once)_
...
- [ ] **Step 7.4: Documentation & merge**
    - [x] **Action:** Update `docs/CHANGELOG.md` with the feature and any deltas (e.g. WASM-vs-frame
    ...
    - [ ] **Verification:** `make` clean build; `git status` review; open a PR onto `biotop`.
        **Status:** `make clean && make` = **zero warnings**; CHANGELOG + ADR done. ⏳ The
        **commit and PR onto `biotop`** are still to be done by the developer.
