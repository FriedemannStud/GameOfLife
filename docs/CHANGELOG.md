2026-06-22 — ADR-0035: Password-protected admin config deletion

- New feature: a single operator can curate the `submissions` collection live from a phone — soft-deleting configs in three selection modes with a mandatory dry-run preview and reversible restore. Soft delete only (`status: active → removed` + `removed_at`/`removed_reason`); the leaderboard already filters on `status:"active"` and the tournament worker self-heals within ≤60 s via its active-roster fingerprint (ADR-0032), so no `match_results` cleanup or manual invalidation is needed.
- Auth: `backend/app/admin_auth.py` — a `require_admin` FastAPI dependency that constant-time-compares (`secrets.compare_digest`) the `X-Admin-Key` header against the new `ADMIN_PASSWORD` env var. Fails closed: an unset/empty secret rejects every request with `401 {"error":"unauthorized"}`. Documented in `.env.example` (placeholder only; the real ≥16-char value lives in the uncommitted `.env`).
- Service layer: `backend/app/admin_service.py` — shared selection logic used identically by dry-run and execute (so the preview can never disagree with what is deleted). Mode A de-duplicates per normalized nickname (keep best by `win_rate` ↓, `avg_stable_generation` ↑, `created_at` ↓; rated beats unrated; singletons untouched); Mode B removes all of a normalized nickname; Mode C resolves a 1-based leaderboard rank to a concrete `submission_id` and deletes by id (immune to worker re-ranking). Plus `soft_delete_ids` (idempotent via a `status:"active"` guard), `list_removed`, `restore_id`.
- API: `backend/app/admin_router.py` (`APIRouter(prefix="/api/admin", dependencies=[Depends(require_admin)])`, wired into `main.py`): `POST /api/admin/delete` (mode dispatch + `dry_run`; 400 on missing mode-B nickname / out-of-range mode-C rank / invalid mode), `GET /api/admin/removed` (trash, newest first), `POST /api/admin/restore` (404 on unknown id). New request models `AdminDeleteRequest`/`AdminRestoreRequest`.
- Admin page: `web/admin/admin.html` (standalone, phone-usable, mirrors the editor's Digital-Lab theme), mounted at `/admin`. Password login gate stores the key in `localStorage` and attaches it as `X-Admin-Key`; a `401` clears the key and returns to the gate. Four tabs — Dedupe (A), By name (B), By rank (C, drives `/api/leaderboard`), Trash — each delete mode using a preview→confirm flow with 8×8 seed thumbnails on a `<canvas>`; a Logout button clears the stored key.
- Tests: `backend/tests/test_admin_service.py` (in-memory async fake DB: mode A/B/C selection, tie-breaks, normalization, idempotency, restore round-trip) and `backend/tests/test_admin_endpoints.py` (live HTTP: 401 on missing/invalid key with no mutation, dry-run preview leaves the DB unchanged, execute transitions rows, trash + restore round-trip; self-seeded and self-cleaning). All pre-existing backend tests still pass; `ruff check .` / `black --check .` clean. The admin page was verified interactively per the Team Principle: login gate (wrong key rejected, correct key reveals the tabs) and all four tabs (Dedupe / By name / By rank / Trash) working end-to-end.
- Docs: `docs/adr/ADR-0035`, `docs/specs/DEV_SPEC-0035`, `docs/tech_design/DEV_TECH_DESIGN-0035`, `docs/tasks/DEV_TASKS-0035`.
- Deployment note: the admin traffic must be served over HTTPS at the booth (the static key is otherwise exposed in the header). Adding `ADMIN_PASSWORD` to `.env` requires recreating the backend container (`docker compose up -d backend`) so the new env var is injected.
...
- `src/core/config.h`: five new named constants (`KIOSK_TARGET_FPS`, `KIOSK_FADE_REF_FPS`, `KIOSK_FADE_PER_REF_FRAME`, `KIOSK_FADE_DT_MAX`, `KIOSK_MAX_FBO_HEIGHT`), replacing two magic numbers. `KIOSK_TARGET_FPS` is the single per-device tuning knob — **committed default 30** (60 originally proposed; lowered after on-device interactive testing confirmed 30 fps is visually acceptable for the attract loop). Raise to 60 for smoother motion on stronger hardware.
- `src/gui/renderer.c`: (FR-1) `SetTargetFPS(120)` → `SetTargetFPS(KIOSK_TARGET_FPS)`. (FR-4) added `FLAG_VSYNC_HINT` to the window flags, removing the busy-wait so the GPU yields between frames. (FR-2) the fossil-trail fade is now frame-rate-independent: `fade = powf(0.95, dt·120)` with `dt` from `GetFrameTime()` clamped to `KIOSK_FADE_DT_MAX`, so trail decay is constant in **wall-clock time** regardless of the cap — at the 120 fps reference `fade = 0.95` exactly (bit-for-bit the old behaviour), and trail length is identical at 30/60/120 fps. (FR-3) the per-quadrant ping-pong FBOs are capped at ≤1080p via the new `compute_fbo_size` helper and bilinear-upscaled on the screen blit; a no-op at ≤1080p, it bounds shader/blit cost on high-DPI/4K panels. `RenderContext` gains `fbo_w`/`fbo_h` (`renderer.h`).
- Net effect: at the default 30 fps cap the sustained Multicam render load is roughly **quartered** versus the old 120 fps baseline and no longer scales with panel resolution beyond 1080p. Verified interactively per the Team Principle (parity at 120 fps, identical trails at 60 and 30, clean window resize). Zero-warning build retained (`-Wall -Wextra -std=c99 -O3 -fopenmp`).
- Build caveat discovered: the Makefile pattern rule depends only on `.c` files, not headers, so the `RenderContext` field additions in `renderer.h` left `app_state_manager.o`/`main.o` stale and caused a struct-layout segfault until `make clean && make`. After any header change, do a clean rebuild.
