...
- [x] **Step 0.1: Search for all highlight pool size references**
    - [x] **Action:** Run the following commands from the project root and read every result:
        ```bash
        ...
        ```
    - [x] **Verification:** Confirm you see exactly these locations (no others):
...
- [x] **Step 1.1: Expand `MatchHighlight matches[4]` in `network_io.h`**
    - [x] **Action:** Open `src/io/network_io.h`. Find line 37:
        ```c
...
- [x] **Step 1.2: Expand the parse loop cap in `network_io.c`**
    - [x] **Action:** Open `src/io/network_io.c`. Find line 147:
        ```c
...
- [x] **Step 1.3: Build and confirm zero warnings**
    - [x] **Action:** Run:
        ```bash
        make
        ```
    - [x] **Verification:** Report the full compiler output.
...
- [x] **Step 2.1: Add `highlight_pool_index` and `highlight_pool_size` to the struct**
    - [x] **Action:** Open `src/gui/app_state_manager.h`. Find the `KioskController` struct (lines 20–34). Locate the line:
        ```c
...
- [x] **Step 2.2: Initialize the new fields in `init_kiosk_controller()`**
    - [x] **Action:** Open `src/gui/app_state_manager.c`. Find `init_kiosk_controller()` (line 51). Locate the line that sets `ctrl->initialized = true;` (line 87). Insert the initialization of the two new fields directly before it:
        ```c
...
- [x] **Step 2.3: Build and confirm zero warnings**
    - [x] **Action:** Run:
        ```bash
        make
        ```
...
- [x] **Step 3.1: Write the static helper function**
    - [x] **Action:** Open `src/gui/app_state_manager.c`. Find line 90 (the line `static double time_since_last_input = 0.0;`). Insert the following new static function **immediately before** that line (i.e., after `init_kiosk_controller` ends at line 88 and the blank line 89):
        ```c
...
- [x] **Step 3.2: Replace the inline loading block in the new-data handler**
    - [x] **Action:** Open `src/gui/app_state_manager.c`. Find the block that currently starts at line 162:
        ```c
            HighlightData hd;
        ...
        ```
    - [x] **Verification:** Build and confirm zero warnings:
        ```bash
        make
        ```
...
- [x] **Step 4.1: Advance the pool index at MULTICAM → LEADERBOARD transition**
    - [x] **Action:** Open `src/gui/app_state_manager.c`. Find the `KIOSK_SUB_MULTICAM` timer block (around line 271):
        ```c
...
- [x] **Step 4.2: Load sims from new pool position at LEADERBOARD → MULTICAM transition**
    - [x] **Action:** Find the `KIOSK_SUB_LEADERBOARD` timer block (around line 254):
        ```c
...
- [x] **Step 4.3: Update the KEY_1..KEY_4 match-select handler**
    - [x] **Action:** Find the keyboard handler (around line 279). The inner loop currently reads seeds from `kiosk_ctrl.cached_highlights.matches[i]`. With round-robin, quadrant `i` displays pool slot `(pool_index + i) % pool_size`. The seed must be read from that same slot.
...
- [x] **Step 4.4: Build — zero warnings required**
    - [x] **Action:** Run:
        ```bash
        make
        ```
...
- [x] **Step 5.1: Start the app and enter Kiosk Mode**
    - [x] **Action:** Start the application:
        ```bash
        ./build/biotope
        ```
...
- [x] **Step 5.2: Observe the second Multicam cycle**
    - [x] **Action:** Let the kiosk run. After 30 s the Multicam view transitions to the Leaderboard (15 s), then back to Multicam. Watch what appears in the second Multicam cycle.
    - [x] **Verification (Interactive Test):**
...
- [x] **Step 5.3: Observe the third Multicam cycle (wrap-around)**
    - [x] **Action:** Let the kiosk continue for one more full cycle (30 s Multicam + 15 s Leaderboard).
    - [x] **Verification (Interactive Test):**
...
- [x] **Step 5.4: Verify the KEY_1 match-select**
    - [x] **Action:** During a Multicam cycle, press key `1` on the keyboard. This should launch a full-screen replay of the match shown in quadrant 1 (top-left).
    - [x] **Verification (Interactive Test):**
...
- [x] **Step 6.1: Update ADR-0024 status to Implemented**
    - [x] **Action:** Open `docs/adr/ADR-0024-highlight-rotation-round-robin.md`. Change:
        ```
        **Status:** Proposed
        ```
        to:
...
- [x] **Step 6.2: Add CHANGELOG entry**
    - [x] **Action:** Open `docs/CHANGELOG.md`. Add the following entry under a new section at the top (after the existing 2026-05-31 entry):
        ```markdown
...
- [x] **Step 6.3: Mark all tasks in this file as done**
    - [x] **Action:** Replace all `- [x]` with `- [x]` in this file once all prior steps are complete.
    - [x] **Verification:** Run:
        ```bash
